...
object ReplyRepository {
  ...
  def repliesForPair(pairId: Long)(implicit session: DBSession): List[ReplyEntity] = withSQL {
    select
      .from(ReplyEntity as r)
      .where.eq(r.pairId, pairId)
      .orderBy(r.count.desc)
      .limit(3)
  }.map(ReplyEntity(r)(_)).list().apply()

  def getReplyOrCreateBy(pairId: Long, wordId: Option[Long])(implicit session: DBSession): ReplyEntity = getReplyBy(pairId, wordId) match {
    case Some(reply: ReplyEntity) => reply
    case None => createReplyBy(pairId, wordId)
  }

  def incrementReply(id: Long, counter: Long)(implicit session: DBSession): Unit = withSQL {
    update(ReplyEntity)
      .set(ReplyEntity.column.count -> (counter + 1))
      .where.eq(ReplyEntity.column.id, id)
  }.update.apply()

  def getReplyBy(pairId: Long, wordId: Option[Long])(implicit session: DBSession): Option[ReplyEntity] = withSQL {
    select
      .from(ReplyEntity as r)
      .where.eq(r.wordId, wordId)
      .and.eq(r.pairId, pairId)
      .limit(1)
  }.map(ReplyEntity(r)(_)).single.apply()
