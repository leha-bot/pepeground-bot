...
object ChatRepository {
  ...
  def getList(limit: Int = 20, offset: Int = 0)(implicit session: DBSession): List[ChatEntity] = withSQL {
    select
      .from(ChatEntity as c)
      .limit(limit)
      .offset(offset)
  }.map(ChatEntity(c)(_)).list().apply()

  def getChatById(id: Long)(implicit session: DBSession): Option[ChatEntity] = withSQL {
    select
      .from(ChatEntity as c)
      .where.eq(c.id, id)
      .limit(1)
  }.map(ChatEntity(c)(_)).single.apply()

  def getOrCreateBy(telegramId: Long, name: String, chatType: String)(implicit session: DBSession): ChatEntity = getByTelegramId(telegramId) match {
    case Some(chat: ChatEntity) => chat
    case None => create(telegramId, name, chatType)
  }

  def updateRandomChance(id: Long, randomChance: Int)(implicit session: DBSession): Unit = withSQL {
    update(ChatEntity)
      .set(
  ...
  def updateRepostChat(id: Long, repostChatUsername: String)(implicit session: DBSession): Unit = withSQL {
    update(ChatEntity)
      .set(
  ...
  def updateChat(id: Long, name: Option[String], telegramId: Long)(implicit session: DBSession): Unit = withSQL {
    update(ChatEntity)
      .set(
  ...
  def getByTelegramId(telegramId: Long)(implicit session: DBSession): Option[ChatEntity] = withSQL {
    select.from(ChatEntity as c)
      .where.eq(c.telegramId, telegramId)
      .limit(1)
  }.map(ChatEntity(c)(_)).single.apply()
}
